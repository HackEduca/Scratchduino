# -*- coding: utf-8 -*-
"""
Original version created on Mon Nov 25 14:45:49 2013
...
class GetHandler(StoppableHttpRequestHandler):
    """
...
#===================================================================================================================
def start_server(firmata, command_handler, scratchduino):
    """
...
#===================================================================================================================
    ...
    try:
        scratchduino.show_msg(2)

        scratchduino.server = StoppableHttpServer(('localhost', 50209), GetHandler)

        #print('Starting HTTP Server!')
        #print('Please start Scratch or Snap!')

        scratchduino.show_msg(3)
        scratchduino.show_msg(4)

    except Exception:
        logging.debug('Exception in scratch_http_server.py: HTTP Socket may already be in use - restart Scratch')
        scratchduino.show_msg(5)
        raise
    try:
        #start the server
        scratchduino.server.serve_forever()

    except KeyboardInterrupt:
        logging.info('scratchduino_http_server.py: keyboard interrupt exception')
        scratchduino.show_msg(6)
        raise KeyboardInterrupt
    except Exception:
        logging.debug('scratch_http_server.py: Exception %s' % str(Exception))
        scratchduino.show('Scratchduino_http_server.py: Exception ' + str(Exception))
        raise