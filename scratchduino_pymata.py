...
from scratchduino_pymata_serial import PyMataSerial
from scratchduino_pymata_command_handler import PyMataCommandHandler
...
class PyMata:
   ...
   # each byte represents a digital port and its value contains the current port settings
    ...
    def __init__(self, frame, port_id='/dev/ttyUSB0'):
        """
        ...
        # reference pointer to the text area on the GUI
        self.frame = frame

        self.frame.show_msg(11)
        frame.show_msg(12)
        frame.text.see(END)
        frame.update_idletasks()
        # Instantiate the serial support class
        self.transport = PyMataSerial(port_id, self.command_deque, frame)
        ...
        self._command_handler = PyMataCommandHandler(self, self.frame)
    ...
    def close(self):
        ...
        self.frame.show_msg(14)
        self.frame.text.see(END)
